...
functions {
  ...
  real partial_sum(array[] int slice_indices,
    ...
    for (i in 1:size(slice_indices)) {
      ...
      // Precompute max mapped evidence (subject-dependent via alpha/beta)
      real m_max = logit(proba_max);
      ...
      for (t in 1:Tsubj[n]) {
        ...
        // --- Influence likelihood (scaled by max possible evidence) ---
        real sub_ev_choice = (ch == 1) ? sub_evidence : -sub_evidence;

        real norm_denom = 1 + S * m_max;
        real sub_scaled = sub_ev_choice / fmax(norm_denom, 1e-6);
...
data {
  ...
  // proba_max = max(largest_proba, 1 - smallest_proba)
  // guarantees proba_max > 0.5, hence logit(proba_max) > 0
  real<lower=0.5, upper=1> proba_max;
...
generated quantities {
  ...
  for (n in 1:N) {
    ...
    // Precompute max mapped evidence for this subject
    real m_max = logit(proba_max);
    ...
    for (t in 1:Tsubj[n]) {
      ...
      // --- scaled influence (normalised by max possible evidence) ---
