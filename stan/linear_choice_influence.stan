functions {
  ...
  real partial_sum(array[] int slice_indices,
	...
	real total_ev_choice = clamp_real((ch == 1) ? ev_diff : -ev_diff, -100, 100);
	real sub_ev_choice   = clamp_real((ch == 1) ? sub_evidence : -sub_evidence, -100, 100);
	...
	real delta_p   = p_with - p_without;
        ...
        if (infl_obs > -90) {
          real mu_infl = clamp_real(a_infl * delta_p + b_infl, -100, 100);
...
generated quantities {
  ...
  for (n in 1:N) {
    ...
    for (t in 1:Tsubj[n]) {
      ...
      y_pred[n, t] = bernoulli_rng(p_blue) + 1;
