  ...
  real partial_sum(array[] int slice_indices,
    ...
    for (i in 1:size(slice_indices)) {
      ...
      for (t in 1:Tsubj[n]) {
        ...
        // Attention-share normalisation
        real total_abs = abs(evidence[1]) + abs(evidence[2]);
        real delta_p_norm = delta_p / (1 + total_abs);

        real infl_obs = influence[n, t];
        if (infl_obs > -90) {
          real mu_infl = a_infl * delta_p_norm + b_infl;
...
generated quantities {
  ...
  for (n in 1:N) {
    ...
    for (t in 1:Tsubj[n]) {
      ...
      // Attention-share normalisation
      real total_abs = abs(ev[1]) + abs(ev[2]);
      real delta_p_norm = delta_p / (1 + total_abs);

      sub_ev_out[n, t] = sub_ev_choice;
      delta_p_out[n, t] = delta_p_norm;

      real ll_infl = 0;
      real infl_obs = influence[n, t];

      if (infl_obs > -90) {
        real mu_infl = params[n, 8] * delta_p_norm + params[n, 9];
